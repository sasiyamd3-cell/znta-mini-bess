...
async (zanta, mek, m, { from, reply, q }) => {
    try {
        ...
        await reply(txt);
    } catch (err) { reply("❌ Database node offline."); }
});

// 2. 🔫 MAFIA 2 CHEAT CODES & TRICKS
cmd({
    pattern: "mafia2",
    alias: ["mafiacheat"],
    react: "🕴️",
...
async (zanta, mek, m, { from, reply }) => {
    try {
        ...
        await reply(txt);
    } catch (err) { reply("❌ Data fetch failure."); }
});

// 3. 🎯 INSULT COMMAND (ගෘප් එකේ එකෙක්ට රිප්ලයි කරලා මඩ්ඩක් ගහන්න)
cmd({
    pattern: "roast",
    alias: ["insult", "madda"],
    react: "🔥",
...
async (zanta, mek, m, { from, reply }) => {
    try {
        ...
        await reply(`🔥 @${target.split('@')[0]}, ${randomRoast}`, { mentions: [target] });
    } catch (err) { reply("❌ Roast matrix failed."); }
});
...
async (zanta, mek, m, { from, reply, sender }) => {
    try {
        ...
        await reply(txt, { mentions: [sender, target] });
    } catch (err) { reply("❌ Logic error."); }
});
...
async (zanta, mek, m, { from, reply }) => {
    try {
        ...
        await reply(`💡 *𝐍𝐄𝐗𝐔𝐒  𝐌𝐀𝐓𝐑𝐈𝐗  𝐀𝐃𝐕𝐈𝐂𝐄:*\n\n> "${randomAdvice}"`);
    } catch (err) { reply("❌ Terminal connection lost."); }
});
...
async (zanta, mek, m, { from, reply }) => {
    try {
        ...
        await reply(txt);
    } catch (err) { reply("❌ System failure."); }
});
...
async (zanta, mek, m, { from, reply, sender }) => {
    try {
        ...
        await reply(`🌈 *𝐆𝐀𝐘  𝐌𝐄𝐓𝐄𝐑  𝐒𝐂𝐀𝐍𝐍𝐄𝐑*\n\n🎯 *Node:* @${target.split('@')[0]}\n📊 *Rate:* \`[ ${rate}% ]\``, { mentions: [target] });
    } catch (err) { reply("❌ Scanner crashed."); }
});
...
async (zanta, mek, m, { from, reply }) => {
    try {
        ...
        await reply(`🪙 *𝐍𝐄𝐗𝐔𝐒  𝐂𝐎𝐈𝐍  𝐅𝐋𝐈𝐏:* \`${side}\``);
    } catch (err) { reply("❌ Gravity node error."); }
});

// 10. 🕵️ SHIP RANDOMLY (ගෘප් එකේ දෙන්නෙක්ව මැච් කරන්න)
cmd({
    pattern: "ship",
    react: "👩‍❤️‍👨",
...
async (zanta, mek, m, { from, isGroup, reply, groupMetadata }) => {
    try {
        ...
        await reply(txt, { mentions: [m1, m2] });
    } catch (err) { reply("❌ Shipping engine failed."); }
});
