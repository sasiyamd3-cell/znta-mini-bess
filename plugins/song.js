...
cmd({
    ...
    alias: ["yta", "mp3", "play"],
    react: "🎧",
    desc: "Download YouTube MP3 using Nexus-API",
    category: "download",
    filename: __filename,
}, async (bot, mek, m, { from, q, reply, userSettings }) => {
    try {
        if (!q) return reply("🎧 *𝐍 𝐄 𝐗 𝐔 𝐒 - 𝐌 𝐃 SONG SEARCH*\n\nExample: .song alone");
        ...
        // 1. API හරහා සෙවීම (මෙතන උඹේ API ලින්ක් එක දාගනින්)
        ...
        let msg = `🎵 *𝐍 𝐄 𝐗 𝐔 𝐒 - 𝐌 𝐃 AUDIO PLAYER* 🎵\n\n` +
                  `📝 *Title:* ${video.title}\n` +
                  `👤 *Author:* ${video.author}\n` +
