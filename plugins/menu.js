...
async (zanta, mek, m, { from, reply, args, userSettings }) => {
    try {
        ...
        // 🛑 NODE PUSH-BACK BYPASS: .menu එකට අදාළ නැති කිසිම මැසේජ් එකක් මේ ෆයිල් එක ඇතුළේ ප්‍රොසෙස් වෙන්න දෙන්නේ නැහැ!
        ...
        if (selectedCategory && groupedCommands[selectedCategory]) {
            ...
            if (groupedCommands[selectedCategory].length === 0) {
                commandList += `  ⚠️  _No commands integrated yet in this node._\n`;
            } else {
                groupedCommands[selectedCategory].forEach((c) => {
                    commandList += `  ⚡  \`${finalPrefix}${c.pattern}\`\n`;
                });
            }
            commandList += `\n> *𝐏𝐎𝐖𝐄𝐑block𝐃 𝐁𝐘 𝐃𝐄𝐕𝐄𝐋𝐎𝐏block𝐑 𝐍block𝐗𝐔𝐒* 🧬`;
        ...
        if (isButtonsOn) {
            ...
            await sendButtons(zanta, from, {
                ...
                text: btnHeader,
                footer: `© 2026 ᴅᴇᴠᴇʟmemoryᴘᴇʀ ɴᴇxᴜꜱ`,
                image: { url: imageLink }, 
                aimode: true,
                buttons: [
                    { id: 'cat_main', text: '📡 ᴍᴀɪɴ' },
                    { id: 'cat_download', text: '📥 ᴅᴏᴡɴʟᴏᴀᴅ' },
                    { id: 'cat_games', text: '🎮 ɢᴀᴍᴇ ɪɴꜰᴏ' },  
            ...
            numHeader += `└───────────────────────┘\n\n` +
                         ...
                         `🧬 *𝐏𝐎𝐖𝐄𝐑𝐄𝐃 𝐁𝐘 𝐃block𝐕block𝐋𝐎𝐏block𝐑 𝐍block𝐗𝐔𝐒*\n` +
        ...
        reply("❌ Terminal Error: " + err.message);
    }
});
