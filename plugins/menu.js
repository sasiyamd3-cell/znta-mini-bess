...
async (zanta, mek, m, { from, reply, args, userSettings, pushname }) => {
    try {
        ...
        // නිකන් මැසේජ් වලට trigger වීම වැලැක්වීම
        ...
        // --- Commands ටික Categories වලට වෙන් කිරීම ---
        ...
        // --- Reply කරපු Number එක හෝ Button ID එක අනුව Category එක තේරීම ---
        ...
        // --- 📂 Category එකක Commands පෙන්වන Logic එක ---
        ...
        // --- 🏠 Main Menu Header එක ---
       let headerText =`${botName.toUpperCase()}\n` +
                                 `┃ Kamathi vidiyakata hadaganna mekath\n` +
                 `╰━━━━━━━━━━━━━━┈⊷\n\n`;

        let imageLink = (settings.botImage && settings.botImage.startsWith("http")) ? settings.botImage : MENU_IMAGE_URL;

        if (isButtonsOn) {
            // --- 🔘 වැඩ කරන GIFTED-BTNS Logic එක ---
            await sendButtons(zanta, from, {
                title: `*${botName}*`, 
                text: headerText + "🔻 ꜱᴇʟᴇᴄᴛ ᴄᴀᴛᴇɢᴏʀʏ ʙᴇʟʟᴏᴡ",
                footer: `© ᴢᴀɴᴛᴀ ᴍɪɴɪ ᴍᴇɴᴜ`,
                image: { url: imageLink }, 
                aimode: true,
                buttons: [
